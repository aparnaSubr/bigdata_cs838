...
ENV="JAVA_HOME=/usr/lib/jvm/java-1.7.0-openjdk-amd64 \
  ...
  YARN_LOG_DIR=/mnt/logs/hadoop \
  YARN_HOME=/home/ubuntu/software/hadoop-${YARN_VERSION} \
  HADOOP_LOG_DIR=/mnt/logs/hadoop \
...
start_hdfs(){
	...
	pdsh -R exec -f $THREADS -w ^instances ssh -o StrictHostKeyChecking=no -o ConnectTimeout=$TIMEOUT %h '( . /home/ubuntu/run.sh -q ; hadoop-daemon.sh start datanode;)'
	hadoop dfsadmin -safemode leave
}

stop_hdfs(){
	printf "\n==== STOP HDFS daemons ! ====\n"
	pdsh -R exec -f $THREADS -w ^instances ssh -o StrictHostKeyChecking=no -o ConnectTimeout=$TIMEOUT %h '( . /home/ubuntu/run.sh -q ; hadoop-daemon.sh stop datanode;)'
...
start_yarn(){
	...
	pdsh -R exec -f $THREADS -w ^instances ssh -o StrictHostKeyChecking=no -o ConnectTimeout=$TIMEOUT %h '( . /home/ubuntu/run.sh -q ; yarn-daemon.sh start nodemanager;)'
}
 
stop_yarn(){
	printf "\n==== STOP YARN daemons ! ====\n"
	pdsh -R exec -f $THREADS -w ^instances ssh -o StrictHostKeyChecking=no -o ConnectTimeout=$TIMEOUT %h '( . /home/ubuntu/run.sh -q ; yarn-daemon.sh stop nodemanager;)'
