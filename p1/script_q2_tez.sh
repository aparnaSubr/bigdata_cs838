...
output_dir="/home/ubuntu/bigdata_cs838/results/q2_${query_id}/$1/cr$6/pc$7/try_$trial/$3"
...
echo "\n##### script_q2_tez.sh $1 q-$2 $3 $4 t-$5 cr-$6 pc-$7 START #####"
...
if [ $4 = "before" ]; then
	...
	if [ $1 = "tez" ]; then
		echo "script_q2_tez.sh : Tez : deleting files in /mnt/logs/apps/ "
		rm -rf /mnt/logs/apps/*
	fi
	...
	if [ $3 = "master" ]; then
		echo "script_q2_tez.sh : Deleting " $workload_dir/output
		rm -rf $workload_dir/output/*
		echo "script_q2_tez.sh : Deleting " $jhist_file_loc
	...
	if [ $3 = "master" ]; then
		###### QUERY
		cd $workload_dir
		sh $workload_dir/q2_run_query_hive_$1.sh $2 $6 $7 > $output_dir/cmdoutput_$2.txt
...
#	echo "newPass123123$" | sudo -S sh -c "sync; echo 3 > /proc/sys/vm/drop_caches"
	...
	if [ $1 = "tez" ]; then
		echo "script_q2_tez.sh : Tez : so copying /mnt/logs/apps/ to output_dir"
		cp -rf /mnt/logs/apps/ $output_dir
	fi
	
	chmod -R 777 $output_dir

	echo "script_q2_tez.sh : output_dir is ====>" $output_dir
	echo "\n##### script_q2_tez.sh $1 q-$2 $3 $4 t-$5 cr-$6 pc-$7 DONE #####"
fi
