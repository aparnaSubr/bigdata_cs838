...
def main():
    ...
    query = windowed_counts.writeStream.outputMode('complete').format('console').option('numRows', '2147483645').option('truncate', 'False').start()
    query.awaitTermination()

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: eventCount.py <hdfs-monitoring-directory>")
        sys.exit(0)
    main()
